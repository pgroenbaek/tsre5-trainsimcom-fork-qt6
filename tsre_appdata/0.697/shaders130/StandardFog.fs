...
void main() {
    if (textureEnabled == 0.0) {
        ...
        vec4 tex2 = texture(uSampler2, vTextureCoord * secondTexEnabled);
        float isSecondTexEnabled = sign(secondTexEnabled);
        fragColor = fragColor * (1.0 - isSecondTexEnabled) + fragColor * tex2 * 2.0 * isSecondTexEnabled;
        ...
        float cosTheta = clamp(dot(normal, lights), 0.0, 1.0);
        ...
        float camdist = clamp(length(shadowPos.xyz), 0.0, 1.0);
        float camdist2 = clamp(length(shadow2Pos.xyz), 0.0, 1.0);
        float t = 1.0 - floor(camdist);
        float t2 = 1.0 - floor(camdist2);

        float shadowsEnabled2 = float(shadowsEnabled);
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            visibility -= shadowsEnabled2 * t * shadowIntensity * 
                          (1.0 - texture(shadow1, vec3(shadowPos2.xy + offset / shadow1Res, shadowPos2.z - bias)));
            visibility -= shadowsEnabled2 * (1.0 - t) * t2 * 0.4 *
                          (1.0 - texture(shadow2, vec3(shadow2Pos2.xy + offset / shadow2Res, shadow2Pos2.z - bias2)));
        }

        // lighting
        vec3 color = diffuseColor.rgb * clamp(visibility, 0.0, 1.0);
        color += ambientColor.rgb;
        fragColor.rgb *= color * colorBrightness;
