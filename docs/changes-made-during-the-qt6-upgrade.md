...
- Easy-to-use build scripts in /scripts.
...
<!-- - TODO: All signal/slot connections have been refactored to use the newer pointer-based syntax that result in compile-time errors. Before they were using the old SIGNAL/SLOT macros that only resulted in run-time errors. Some connections were broken after the Qt6 upgrade and the easiest way to ensure all of them worked was to have them throw errors during compilation. All broken connections have been fixed. -->

## Changes not related to Qt6 specifically
- GLUU.cpp: Rather than hardcoding shader version per platform, TSRE now asks OpenGL what version of shader it supports and will select version 130 or 330 based on that.
...
## Things removed
- OpenAL headers in the /AL directory (fetching these is now handled by cmake and vcpkg the build folder).
- Auto-generated moc_*.cpp files (handled by cmake in the build folder).
