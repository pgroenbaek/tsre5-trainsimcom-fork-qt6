...
that potentially could be broken (and many are). The old SIGNAL/SLOT macros only result in errors during runtime when broken.
Therefore changing them to the newer pointer-based syntax that result in compile-time errors is probably the best
way to make sure everything still works. This is easy enough to do but will take time due to the sheer number of connections.
